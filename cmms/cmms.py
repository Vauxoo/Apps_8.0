...
class cmms_equipment(osv.osv):
    ...
    def create(self, cr, user, vals, context=None):
        if ('type' not in vals) or (vals.get('type') == '/'):
            vals['type'] = self.pool.get('ir.sequence').get(cr, user,
                                                            'cmms.equipment')
    ...
    def _count_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        return {
            equipment_id: {
                'pm_count': Logpm.search_count(cr, uid,
                                               [('equipment_id', '=',
                                                equipment_id)],
                                               context=context),
                'cm_count': Logcm.search_count(cr, uid,
                                               [('equipment_id', '=',
                                                equipment_id)],
                                               context=context),
                'order_count': Logorder.search_count(cr, uid,
                                                     [('equipment_id', '=',
                                                      equipment_id)],
                                                     context=context),
                'intervention_count': Logintervention.search_count(
                    cr, uid, [('equipment_id', '=', equipment_id)],
                    context=context)
            }
    ...
    def return_action_to_open(self, cr, uid, ids, context=None):
        """ This opens the xml view specified in xml_id for the
         current machine"""
        if context is None:
            context = {}
        if context.get('xml_id'):
            res = self.pool.get('ir.actions.act_window').for_xml_id(
                cr, uid, 'cmms', context['xml_id'], context=context)
            res['context'] = context
    ...
    _columns = {
        ...
        'intervention_id': fields.one2many('cmms.intervention',
                                           'equipment_id', 'Interventions'),
        'cm_id': fields.one2many('cmms.cm', 'equipment_id',
                                 'Maintenance Corrective '),
        'pm_id': fields.one2many('cmms.pm', 'equipment_id',
                                 'Maintenance Preventive'),
        'user_id': fields.many2one('res.users', 'Manager'),
        'photo': fields.binary('Photo'),
        'intervention_count': fields.function(_count_all,
                                              type='integer',
                                              string='Intervention',
                                              multi=True),
        'pm_count': fields.function(_count_all,
                                    type='integer',
                                    string='Preventive maintenance',
                                    multi=True),
        'cm_count': fields.function(_count_all,
                                    type='integer',
                                    string='Corrective maintenance',
                                    multi=True),
        'order_count': fields.function(_count_all,
                                       type='integer',
                                       string='Work Order',
                                       multi=True),
    }
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default['type'] = self.pool.get('ir.sequence').get(cr, uid,
                                                           'cmms.equipment')
        return super(cmms_equipment, self).copy(cr, uid, id, default=default,
                                                context=context)
...
class cmms_intervention(osv.osv):
    ...
    _columns = {
        ...
        'equipment_id': fields.many2one('cmms.equipment',
                                        'Unit of work',
                                        required=True),
        ...
        'type': fields.selection([('check', 'Check'),
                                  ('repair', 'Repair'),
                                  ('revision', 'Revision'),
                                  ('other', 'Other')],
                                 'Intervention type', size=32),
        'state_machine': fields.selection([('start', 'En Marche'),
                                           ('stop', 'En Arret')],
                                          'Etat de machine', size=32),
        'state': fields.selection([('draft', u'En cours'),
                                   ('done', u'Validé'),
                                   ('cancel', u'Annulé')],
                                  u'Statut', required=True),
    }
    _defaults = {
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        if default is None:
            default = {}
            default = default.copy()
            default['name'] = self.pool.get('ir.sequence').get(
                cr, uid, 'cmms.intervention')
        return super(cmms_intervention, self).copy(
            cr, uid, id, default=default, context=context)

    """email"""
    def action_broadcast(self, cr, uid, ids, context={}):
        data_email = []
        text_inter = u"""<div style="font-family: 'Lucica Grande', Ubuntu,
                Arial, Verdana, sans-serif; font-size: 12px; color:
                rgb(34, 34, 34); background-color: rgb(255, 255, 255); ">
                <p>Bonjour %s, </p>
                <p>Nous vous informons que vous êtes attribué a l'intervention
                 %s de l'émetteur  %s.</p>
                <br/>
                <p></p>
                <p>-----------------------------</p>
        ...
        for object_inter in self.browse(cr, uid, ids):
            if not object_inter.user2_id.login:
                raise osv.except_osv(u'Email non spécifiée',
                                     u'Veuillez indiquer l\'email\
                                      de Destinataire')
        ...
        self.pool.get('cmms.parameter.mail').send_email(
            cr, uid, data_email, module='cmms', param='cmms_event_mail')
...
class cmms_incident(osv.osv):
    ...
    _columns = {
        ...
        'active': fields.boolean('Active?'),
        'ref': fields.reference('Work order source',
                                selection=_links_get, size=128),
        'equipment_id': fields.many2one('cmms.equipment',
                                        'Unit of work', required=True),
        'archiving3_ids': fields.one2many('cmms.archiving3',
                                          'incident_id', 'follow-up history'),
    }
    _defaults = {
        'active': lambda * a: True,
        'name': lambda self, cr, uid, context: '/',
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default = default.copy()
        default['name'] = self.pool.get('ir.sequence').get(
            cr, uid, 'cmms.incident')
        return super(cmms_incident, self).copy(
            cr, uid, id, default=default, context=context)
...
class cmms_archiving3(osv.osv):
    ...
    _columns = {
        ...
        'incident_id': fields.many2one(
            'cmms.incident', 'Incident', required=True),
        'user_id': fields.many2one('res.users', 'Manager', readonly=True),
    }
    _defaults = {
        'date': lambda *a: time.strftime('%Y-%m-%d %H:%M:%S'),
...
class cmms_pm(osv.osv):
        ...
        if ids:
            ...
            for record in reads:
                if (record.meter == "days"):
                    ...
                    last_done = datetime.datetime.fromtimestamp(
                        time.mktime(time.strptime(last_done, "%Y-%m-%d")))
    ...
    _columns = {
        ...
        'recurrent': fields.boolean('Recurrent ?',
                                    help="Mark this option if PM is periodic"),
        'days_interval': fields.integer('Interval'),
        'days_last_done': fields.date('Begun the', required=True),
        'days_next_due': fields.function(_days_next_due, method=True,
                                         type="date", string='Next date'),
        'days_warn_period': fields.integer('Warning date'),
        'user_id': fields.many2one('res.users', 'Chef'),
        'days_left': fields.function(_days_due, method=True,
                                     type="integer", string='Staying days'),
        'state': fields.function(_get_state, method=True,
                                 type="char", string='Status')
    }
    _defaults = {
        'meter': lambda * a: 'days',
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        if default is None:
            ...
            default['name'] = self.pool.get('ir.sequence').get(
                cr, uid, 'cmms.pm')
        return super(cmms_pm, self).copy(
            cr, uid, id, default=default, context=context)
cmms_pm()
...
class cmms_checklist(osv.osv):
    ...
    _columns = {
        ...
        'description': fields.text('Description'),
        'questions_ids': fields.one2many("cmms.question",
                                         "checklist_id", "Questions",),
...
class cmms_question(osv.osv):
    ...
    _columns = {
        ...
        'checklist_id': fields.many2one('cmms.checklist',
...
class cmms_checklist_history(osv.osv):
    ...
    def onchange_checklist_id(self, cr, uid, ids, id, context={}):
        liste = self.pool.get('cmms.question').search(
            cr, uid, [('checklist_id', '=', id)])
    ...
    _columns = {
        ...
        'answers_ids': fields.one2many("cmms.answer.history",
                                       "checklist_history_id", "Responses"),
...
class cmms_question_history(osv.osv):
    ...
    _columns = {
        'name': fields.char("Question", size=128, required=True),
        'checklist_history_id': fields.many2one(
            'cmms.checklist.history', 'Checklist'),
...
class cmms_cm(osv.osv):
    ...
    _columns = {
        ...
        'equipment_id': fields.many2one(
            'cmms.equipment', 'Unit of work', required=True),
        'failure_id': fields.many2one('cmms.failure', 'Failure?'),
        'date': fields.datetime('Date'),
        'note': fields.text('Notes'),
        'user_id': fields.many2one('res.users', 'Chef'),
        'diagnosistab_ids': fields.one2many(
            'cmms.diagnosistab', 'cm_id', 'Diagnosis Table'),
    }
    _defaults = {
        'name': lambda self, cr, uid, context: '/',
        'date': lambda *a: time.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(cmms_cm, self).copy(
            cr, uid, id, default=default, context=context)
