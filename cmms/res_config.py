...
class cmms_parameter_mail(osv.osv):
    ...
    def get_value_by_reference(self, cr,
                               uid, module, reference_param, context={}):
        u"""récupère la valeur du paramètre"""
        if reference_param:
            object_model_data = self.pool.get('ir.model.data')
            if object_model_data:
                param_id = False
                param_model, param_id = object_model_data.get_object_reference(
                    cr, uid, module, reference_param)
                if param_id:
                    object_param = self.browse(cr, uid, param_id)
                    if object_param:
                        return [object_param.server and
                                object_param.server.id or
                                False, object_param.mail_from or
                                object_param.server.smtp_user]
        return []
    ...
    def create(self, cr, user, vals, context=None):
        ...
        if flag_travel:
            return super(cmms_parameter_mail, self).create(
                cr, user, vals, context)
        else:
            raise osv.except_osv(_(
                u'Création impossible'), _(
                u'Vous ne pouvez pas créer de paramètres, \
                ils sont définis nativement'))

    def copy(self, cr, uid, id, default=None, context={}):
        u"""méthode de copie"""
        raise osv.except_osv(_(
            u'Copie impossible'), _(
            u'Vous ne pouvez pas copier de paramètres, \
            ils sont définis nativement'))

    def unlink(self, cr, uid, ids, context=None):
        u"""méthode de suppression"""
        raise osv.except_osv(_(
            u'Suppression impossible'), _(
            u'Vous ne pouvez pas supprimer de paramètres, \
            ils sont définis nativement'))

    def send_email(self, cr, uid, data_emails, sender=False, server=False,
                   partner=False, module=False, param=False,
                   auto_commit=False):
        mail_server = False
        mail_from = False
        ids_mail = []
        ...
        for mail in data_emails:
            ids_mail.append(self.pool.get('mail.mail').create(
                cr, uid, {
...
class cmms_parameter_option(osv.osv):
    ...
    def get_value_by_reference(
            self, cr, uid, module, reference_param, context={}):
        u"""récupère la valeur du paramètre"""
        if reference_param:
            object_model_data = self.pool.get('ir.model.data')
            if object_model_data:
    ...
    def create(self, cr, user, vals, context=None):
        ...
        flag_travel = vals.get('is_param', False)
        if flag_travel:
            return super(cmms_parameter_option, self).create(
                cr, user, vals, context)
        else:
            raise osv.except_osv(_(
                u'Création impossible'), _(
...
class cmms_config_settings(osv.osv_memory):
    ...
    def create(self, cr, uid, vals, context={}):
        res = self.pool.get('ir.model.data').get_object_reference(
            cr, uid, 'cmms', 'cmms_event_mail')
        if res and res[1]:
            if 'server_cmms' in vals:
                self.pool.get('cmms.parameter.mail').write(
                    cr, uid, res[1], {'server': vals['server_cmms']})
    ...
    def _get_server_evaluation(self, cr, uid, context={}):
        ...
        try:
            res = self.pool.get('ir.model.data').get_object_reference(
                cr, uid, 'cmms', 'cmms_event_mail')
            if res and res[1]:
                data_read = self.pool.get('cmms.parameter.mail').read(
                    cr, uid, res[1], ['server'])
                if data_read:
                    return data_read['server'] and data_read[
                        'server'][0] or False
        except:
            pass
        return False
